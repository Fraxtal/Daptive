...
namespace Daptive.Admin
{
    public partial class Courses : Page
    {
                ...
                using (SqlConnection conn = new SqlConnection(_connStr))
                {
                    ...
                    string sql = @"
                        SELECT c.CourseId,
                               ...
                               c.Content,
                               c.DefaultCode,
                               ISNULL(t.Topic, 'No Topic') AS TopicName
                        FROM   [course] c
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(_connStr))
                {
                    ...
                    string sql = @"
                        ...
                        FROM   [topic] t
                        LEFT JOIN [course] c ON t.TopicId = c.TopicId
                        GROUP BY t.TopicId, t.Topic, t.Description
        ...
        // ── Course Repeater Command ───────────────────────────────────
        protected void rptCourses_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName != "DeleteCourse") return;
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(_connStr))
                {
                    ...
                    // Delete related questions and their test cases / scores first
                    string deleteScores = @"
                        DELETE FROM [score]
                        WHERE QuestionId IN (
                            SELECT QuestionId FROM [question] WHERE QuestionId = @CourseId
                        )";
        ...
        protected void rptTopics_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(_connStr))
                {
                    ...
                    // Nullify the TopicId on courses that use this topic
                    // rather than deleting the courses themselves
