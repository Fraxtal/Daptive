            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(_connStr))
                {
                    ...
                    string sql = @"
                        ...
                        LEFT JOIN [question] q ON s.QuestionId = q.QuestionId
                    ...
                    // Add a colour class based on score value
                    dt.Columns.Add("ScoreClass", typeof(string));
                    foreach (DataRow row in dt.Rows)
                    {
                        int score = row["Score"] != DBNull.Value ? Convert.ToInt32(row["Score"]) : 0;
                        if (score >= 80) row["ScoreClass"] = "score-high";
                        else if (score >= 50) row["ScoreClass"] = "score-mid";
                        else row["ScoreClass"] = "score-low";
                    }
